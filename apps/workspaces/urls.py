...
urlpatterns = [
    ...
    path(
        "<int:workspace_id>/connect_xero/authorization_code/", ConnectXeroView.as_view()
    ),
    path("<int:workspace_id>/credentials/xero/", ConnectXeroView.as_view()),
    path(
        "<int:workspace_id>/connection/xero/revoke/", RevokeXeroConnectionView.as_view()
    ),
    path(
        "<int:workspace_id>/exports/trigger/",
        ExportToXeroView.as_view(),
        name="export-to-xero",
    ),
