...
def assert_4xx_cases(api_client, url, data):
    ...
    assert "Settings are required" in str(
        response.data
    ), "clone settings api returns a diff in the message"
...
def test_clone_settings(api_client, test_connection):
    ...
    response = api_client.put(url, data=data["clone_settings"], format="json")

    assert response.status_code == 200
    response = json.loads(response.content)
    assert (
        dict_compare_keys(response, data["clone_settings_response"]) == []
    ), "clone settings api returns a diff in the keys"

    response = api_client.put(
        url, data=data["clone_settings_missing_values"], format="json"
    )
...
def test_4xx_import_settings(api_client, test_connection):
    ...
    assert_4xx_cases(
        api_client, url, {"export_settings": data["clone_settings"]["export_settings"]}
    )
...
def test_clone_settings_exists(api_client, test_connection):
    ...
    response = api_client.get(url, data=data["clone_settings_exists"], format="json")

    assert response.status_code == 200
    response = json.loads(response.content)
    assert (
        dict_compare_keys(response, data["clone_settings_exists"]) == []
    ), "clone settings api returns a diff in the keys"
    ...
    response = json.loads(response.content)
    assert (
        dict_compare_keys(response, data["clone_settings_not_exists"]) == []
    ), "clone settings api returns a diff in the keys"
