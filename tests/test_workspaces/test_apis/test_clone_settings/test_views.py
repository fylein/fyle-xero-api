...
def test_clone_settings_availability(api_client, test_connection):
    workspace = Workspace.objects.get(id=1)
    workspace.onboarding_state = 'COMPLETE'
    workspace.save()

    url = '/api/user/clone_settings/availability/'
    api_client.credentials(HTTP_AUTHORIZATION='Bearer {}'.format(test_connection.access_token))
    response = api_client.get(
        url,
        data=data['clone_settings_availability'],
        format='json'
    )

    assert response.status_code == 200
    response = json.loads(response.content)
    assert dict_compare_keys(response, data['clone_settings_availability']) == [], 'clone settings api returns a diff in the keys'
    ...
    assert response.status_code == 200
    response = json.loads(response.content)
    assert dict_compare_keys(response, data['clone_settings_un_availability']) == [], 'clone settings api returns a diff in the keys'
