...
def test_advanced_settings(api_client, test_connection):
    ...
    assert dict_compare_keys(response, data['advanced_settings_response']) == [], 'advanced settings api returns a diff in the keys'

    response = api_client.put(
        url,
        data=data['advanced_settings_missing_values'],
        format='json'
    )
