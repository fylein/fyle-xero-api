...
def test_export_settings(api_client, test_connection):
    ...
    assert (
        dict_compare_keys(response, data["export_settings_response"]) == []
    ), "Export settings api returns a diff in the keys"

    response = api_client.put(
        url, data=data["export_settings_missing_values"], format="json"
    )
