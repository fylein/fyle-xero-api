...
def test_auto_create_tax_codes_mappings(db, mocker):
    ...
    mocker.patch(
        "xerosdk.apis.TaxRates.get_all", return_value=xero_data["get_all_tax_codes"]
    )
...
def test_auto_create_project_mappings(db, mocker):
    ...
    mocker.patch("apps.xero.utils.XeroConnector.sync_customers", return_value=[])
...
def test_upload_categories_to_fyle(db, mocker):
    ...
    mocker.patch("apps.xero.utils.XeroConnector.sync_accounts", return_value=[])

    workspace_id = 1

    da_instance = DestinationAttribute.objects.filter(
...
def test_auto_create_cost_center_mappings(db, mocker, create_mapping_setting):
    ...
    with mock.patch(
        ...
        mock_call.side_effect = FyleInvalidTokenError(
            msg="Invalid Token for Fyle", response="Inalid Token for Fyle"
        )
        response = auto_create_cost_center_mappings(workspace_id=workspace_id)
